...
FTFL1 <- function(fdata_, k, lam, L, ep1, ep2) {
...
# KL       ... Karhunen–Loève estimation of trend of class "fdata"
 ...
 #  Check fdata_
  ...
  while (er > ep1) {
    ...
    for (i in 1:end) {
      c <- ea[i,]%*%C%*%b+u[i,]/rho
      a[i,] <- pmin(pmax(c-lam/rho,0),c+lam/rho)
    }
...
FTFL1_select <- function(fdata_, k, L, ep1, ep2, candidates) {
  ...
  for (lam in candidates) {
    ...
    for (p in 1:(d2-1)) {
      ...
      result <- FTFL1(fdata(fdata_train), k, lam, L, ep1, ep2)
